...
| GET | `/tenants/invitations` | `Tenants.View` | List new-tenant creation invitations (filter by `status`). |
| POST | `/tenants/invite` | `Tenants.Create` | Send new-tenant invitation by email. Admin enters email only; invited user sets up tenant name, slug, addresses, and password via the invitation link. |
