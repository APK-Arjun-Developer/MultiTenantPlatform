...
| Method | Path | Permission | Notes |
|--------|------|------------|-------|
| GET | `/tenant-admins` | `Tenants.List` | List all TenantAdmins; filter by `tenantId`, `isActive`, `createdVia`; sort by `fullName`, `email` |
...
`GET /users`, `GET /tenants`, `GET /roles`, `GET /tenant-admins`, `GET /activity-logs`:
...
`GET /users`, `GET /tenants`, `GET /tenant-admins`, `GET /roles`, and `GET /activity-logs` all accept optional sort params:

| Query | Type | Notes |
|-------|------|-------|
| `sortBy` | `string` | Column key to sort by (see per-endpoint table below) |
| `sortOrder` | `asc` \| `desc` | Default is endpoint-specific when omitted |

Supported `sortBy` values per endpoint:

| Endpoint | Supported keys | Default order |
|----------|----------------|---------------|
| `GET /users` | `fullName`, `email`, `lastLoginAt` | `email` asc |
| `GET /tenants` | `name` | `name` asc |
| `GET /tenant-admins` | `fullName`, `email` | `email` asc |
| `GET /roles` | `name` | `name` asc |
| `GET /activity-logs` | `createdAt` | `createdAt` desc |
...
Query params: `page`, `pageSize`, `userId` (GUID), `module`, `action`, `dateFrom` (ISO), `dateTo` (ISO), `sortBy` (`createdAt`), `sortOrder` (`asc`|`desc`, default `desc`).
